...
import '../charts/monthly_setor_chart2.dart';
...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  List<String> _selectedBusinessOwners = [];

  List<String> _kodeMapOptions = [];
  List<String> _selectedKodeMaps = [];
  ...
  Future<void> _loadAllDashboardData() async {
    if (_selectedKpp == null || _selectedTahun == null) return;

    // Load cube & revenue data (otomatis membangun semua monthly cache)
    await _dashboardDataService.loadRevenueData(_selectedKpp!, _selectedTahun!);
                ...
                children: [
                  ...
                  IntrinsicWidth(
                    child: OutlinedButton(
                      onPressed: () async {
                        final result = await showDialog<List<String>>(context: context, builder: (ctx) {
                          // default to all when internal selection is empty (meaning "no filter")
                          final initialSet = _selectedBusinessOwners.isNotEmpty ? Set<String>.from(_selectedBusinessOwners) : Set<String>.from(_businessOwnerOptions);
                            final selected = Set<String>.from(initialSet);
                            final ScrollController _boScrollController = ScrollController();
                            return StatefulBuilder(
                              builder: (ctx2, setState) {
                              return AlertDialog(
                                    title: const Text('Pilih Business Owner'),
                                    content: SizedBox(
                                      width: 360.0,
                                      height: MediaQuery.of(ctx2).size.height * 0.45,
                                      child: Column(
                                        ...
                                        children: [
                                          Row(
                                            ...
                                            children: [
                                              ...
                                              TextButton(
                                                onPressed: _businessOwnerOptions.isNotEmpty
                                                    ? () {
                                                        setState(() {
                                                          if (selected.length == _businessOwnerOptions.length) {
                                                            // clear all
                                                            selected.clear();
                                                          } else {
                                                            // select all
                                                            selected.clear();
                                                            selected.addAll(_businessOwnerOptions);
                                                          }
                                                        });
                                                      }
                                                    : null,
                                                child: Text(selected.length == _businessOwnerOptions.length ? 'Batal Semua' : 'Pilih Semua'),
                                              ),
                                            ],
                                          ),
                                          ...
                                          Expanded(
                                            child: Scrollbar(
                                              controller: _boScrollController,
                                              thumbVisibility: true,
                                              child: ListView(
                                                controller: _boScrollController,
                                                children: _businessOwnerOptions.map((opt) {
                                    ...
                                    actions: [
                                      ...
                                      ElevatedButton(onPressed: () {
                                        // If user selected all options, treat as no-filter (store empty list)
                                        if (selected.length == _businessOwnerOptions.length) Navigator.of(ctx).pop(<String>[]);
                                        else Navigator.of(ctx).pop(selected.toList());
                                      }, child: const Text('OK')),
                                    ],
                                  );
                            },
                        ...
                        if (result != null) {
                          setState(() {
                            _selectedBusinessOwners = result;
                          });
                        }
                      },
                      child: Text(_selectedBusinessOwners.isEmpty ? 'Business Owner' : _selectedBusinessOwners.join(', ')),
                    ),
                  ),
                  const SizedBox(width: 16),
                  IntrinsicWidth(
                    child: OutlinedButton(
                      onPressed: () async {
                        final result = await showDialog<List<String>>(context: context, builder: (ctx) {
                          final initialSet = _selectedKodeMaps.isNotEmpty ? Set<String>.from(_selectedKodeMaps) : Set<String>.from(_kodeMapOptions);
                          final selected = Set<String>.from(initialSet);
                          final ScrollController _kdScrollController = ScrollController();
                                return StatefulBuilder(
                            builder: (ctx2, setState) {
                              return AlertDialog(
                                title: const Text('Pilih Kode MAP'),
                                content: SizedBox(
                                  // constrain the dialog list height so it doesn't grow too tall
                                  width: 360.0,
                                  height: MediaQuery.of(ctx2).size.height * 0.45,
                                  child: Column(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          const SizedBox(width: 8),
                                          TextButton(
                                            onPressed: _kodeMapOptions.isNotEmpty
                                                ? () {
                                                    setState(() {
                                                      if (selected.length == _kodeMapOptions.length) {
                                                        selected.clear();
                                                      } else {
                                                        selected.clear();
                                                        selected.addAll(_kodeMapOptions);
                                                      }
                                                    });
                                                  }
                                                : null,
                                            child: Text(selected.length == _kodeMapOptions.length ? 'Batal Semua' : 'Pilih Semua'),
                                          ),
                                        ],
                                      ),
                                      const SizedBox(height: 8),
                                      Expanded(
                                        child: Scrollbar(
                                          controller: _kdScrollController,
                                          thumbVisibility: true,
                                          child: ListView(
                                            controller: _kdScrollController,
                                            children: _kodeMapOptions.map((opt) {
                                              return CheckboxListTile(
                                                value: selected.contains(opt),
                                                title: Text(opt),
                                                onChanged: (v) {
                                ...
                                actions: [
                                  TextButton(onPressed: () => Navigator.of(ctx).pop(null), child: const Text('Batal')),
                                  ElevatedButton(onPressed: () {
                                    if (selected.length == _kodeMapOptions.length) Navigator.of(ctx).pop(<String>[]);
                                    else Navigator.of(ctx).pop(selected.toList());
                                  }, child: const Text('OK')),
                                ],
                              );
                        ...
                        if (result != null) {
                          setState(() {
                            _selectedKodeMaps = result;
                          });
                        }
                      },
                      child: Text(_selectedKodeMaps.isEmpty ? 'Kode MAP' : _selectedKodeMaps.join(', ')),
                    ),
                  ),
                  ElevatedButton.icon(
                    onPressed: () async {
                      ...
                      child: Column(
                        children: [
                          ExpansionTile(
                            ...
                            children: [
                              Builder(builder: (ctx) {
                                // dataset flagKey handled by service (PKPM -> FLAG_PKPM, VOLUNTARY -> VOLUNTARY)

                                // If Business Owner or Kode MAP multi-select filter is selected, request aggregated data from service
                                List<Map<String, dynamic>> chartSourceData;
                                final boSelected = _selectedBusinessOwners.isNotEmpty;
                                final kdMapSelected = _selectedKodeMaps.isNotEmpty;
                                if (boSelected || kdMapSelected) {
                                  final Map<String, dynamic> filters = {};
                                  if (boSelected) filters['FLAG_BO'] = _selectedBusinessOwners;
                                  if (kdMapSelected) filters['KD_MAP'] = _selectedKodeMaps;
                                  chartSourceData = _dashboardDataService.monthlyDataForDataset(_selectedDataset, filters: filters);
                                } else {
                                  chartSourceData = _dashboardDataService.monthlyDataForDataset(_selectedDataset);
                                }

                                return MonthlySetorChart(
                                  ...
                                  selectedDatasetLabel: (AppConstants.datasetLabels[_selectedDataset] ?? _selectedDataset) +
                                      (_selectedBusinessOwners.isNotEmpty ? ' - ' + _selectedBusinessOwners.join(', ') : ''),
                                  dashboardData: chartSourceData,
                                  renpenCube: _dashboardDataService.renpenCube,
                                  selectedBusinessOwner: _selectedBusinessOwners.isNotEmpty ? _selectedBusinessOwners.join(', ') : null,
                                  selectedKodeMap: _selectedKodeMaps.isNotEmpty ? _selectedKodeMaps.join(', ') : null,
