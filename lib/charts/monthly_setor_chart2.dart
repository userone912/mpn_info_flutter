...
class MonthlySetorChart extends StatefulWidget {
  ...
  final Map<String, double> renpenCube;
  final String? selectedBusinessOwner;
  final String? selectedKodeMap;
  ...
  const MonthlySetorChart({
    ...
    required this.renpenCube,
    required this.selectedBusinessOwner,
    required this.selectedKodeMap,
...
class _MonthlySetorChartState extends State<MonthlySetorChart> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.selectedBusinessOwner != null && widget.selectedBusinessOwner!.isNotEmpty) {
      // Support multi-selected business owners passed as comma-joined string.
      final selSet = widget.selectedBusinessOwner!
          .split(',')
          .map((s) => s.trim())
      ...
      if (selSet.isNotEmpty) {
        data = data.where((row) {
          final rowVal = (row['FLAG_BO'] ?? '').toString();
          final rowSet = rowVal
              .split(',')
              .map((s) => s.trim())
              .where((s) => s.isNotEmpty)
              .toSet();
          return rowSet.intersection(selSet).isNotEmpty;
        }).toList();
      }
    }
    // Apply KD_MAP multi-select filter (comma-joined values) if provided
    if (widget.selectedKodeMap != null && widget.selectedKodeMap!.isNotEmpty) {
      final selKdSet = widget.selectedKodeMap!
          .split(',')
          .map((s) => s.trim())
          .where((s) => s.isNotEmpty)
          .toSet();
      if (selKdSet.isNotEmpty) {
        data = data.where((row) {
          final rowVal = (row['KD_MAP'] ?? '').toString();
          final rowSet = rowVal
              .split(',')
              .map((s) => s.trim())
              .where((s) => s.isNotEmpty)
              .toSet();
          return rowSet.intersection(selKdSet).isNotEmpty;
        }).toList();
      }
    }
    // Integrate RENPEN cube for REN aggregation by selectedKdmap
    // Assume DashboardDataService is accessible as a singleton or via Provider
  final renpenCube = widget.renpenCube;
    final Set<String> selKdSet = (widget.selectedKodeMap != null && widget.selectedKodeMap!.isNotEmpty)
        ? widget.selectedKodeMap!.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toSet()
        : {};
    // Prepare Renpen monthly map: {bln: total_target}, sum all matching KD_MAP (ignore NIP for now)
    final Map<String, double> renpenMonthly = {};
    renpenCube.forEach((key, value) {
      final parts = key.split('|'); // [BLN_SETOR, KD_MAP, NIP]
      if (parts.length < 2) return;
      final bln = parts[0];
      final kdmap = parts[1];
      if (selKdSet.isEmpty || selKdSet.contains(kdmap)) {
        renpenMonthly[bln] = (renpenMonthly[bln] ?? 0) + value;
      }
    });
    String flagKey;
  ...
  if (data.isEmpty && renpenMonthly.isEmpty) {
      // LayoutBuilder lets this widget size itself to the available
      // space provided by the parent. When the chart area provides a
      // finite maxHeight the spinner will center vertically inside it.
      chartWidget = LayoutBuilder(
        builder: (ctx, c) {
          // If parent supplies a finite height, use it; otherwise fall
          // back to a reasonable fraction of screen height.
          final double height = (c.hasBoundedHeight && c.maxHeight > 0)
              ? c.maxHeight
      ...
      print('_monthlyData: ${widget.dashboardData.length}');
      ...
      // renpenMonthly already prepared above from renpenCube
